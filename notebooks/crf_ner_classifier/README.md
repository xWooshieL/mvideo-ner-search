...
```text
query → SpellFixer → WeakLabeler (silver BIO) → CRF → spans
                                      └→ ATTR span → attr_type_clf
query → brand_clf (fallback / отдельный трек)
```

Опечатки (`ноутбок`→`ноутбук`, `16гь`→`16 гб`) чинит `SpellFixer` **до** разметки/CRF.
Один и тот же слой: в `_run_01` (сбор silver) и в `extractor` (прод).
Gold (`bio_liza.jsonl`) для eval **не** переписываем — теги уже на исходных токенах.

## Обучение сейчас (для друга)

```bash
# 1) пересобрать silver уже со spellfix
python notebooks/crf_ner_classifier/_run_01.py

# 2) обучить CRF на новом silver → models/ner_crf.pkl
python notebooks/crf_ner_classifier/_run_02.py
```

Или ноутбуки `01_crf_eda.ipynb` / `02_crf_classifier.ipynb` — если крутите руками,
перед `label_query` добавьте то же:

```python
from src.preprocessing import SpellFixer
spell = SpellFixer.from_artifacts("artifacts")
q, _ = spell.fix_query(q)
```

Headless `_run_01` уже делает это сам.
...
SpellFix: [`src/preprocessing/spellfix.py`](../../src/preprocessing/spellfix.py).
