...
Фокус дальше — не «ещё один NER ради NER», а **починить грязный ввод** и **достроить смысл голых чисел**. Это напрямую бьёт в качество фактов и в выдачу.

### ✅ Что уже сделано (день 4–5)

| Что | Где | Статус |
|---|---|---|
| **SpellFixer** перед CRF/serve (`гь→гб`, fuzzy по словарю) | `src/preprocessing/spellfix.py`, хук в `_run_01` CRF + `extractor` | ✅ |
| **SpellFix v2**: гомоглифы `c/с`, `a/а`… + алиасы транслита (`сони→sony`, `плейстейшен→playstation`) | `spellfix.py` + `artifacts/dicts/spell_aliases.txt` | ✅ |
| **CRF** пересобран после spellfix (журнал друга) | `artifacts/history/ner_crf.md` — gold microF1 ~0.59 | ✅ |
| **GLiNER** zero-shot + fine-tune (`urchade/gliner_multi-v2.1`) | `notebooks/gliner/` — отчёты + `artifacts/metrics/gliner_*.json` | ✅ эксперимент |
| **Joint study**: один silver → CRF+GLiNER, метрика каскада rules→CRF→GLiNER на gold / broken / silver-val | `notebooks/general_study/` | ✅ эксперимент |
| Журнал экспериментов + broken queries | `artifacts/history/` | ✅ (Некит) |

**Важно:** в прод-`extractor` сейчас `SpellFix → rules → CRF` (+ fuzzy/brand/attr). **GLiNER в прод ещё не вшит** — только ноутбуки/`models/general_study/` и `models/gliner_ner/` (локально, не в git).

Честные ориентиры: **gold microF1 ~0.58–0.64** (каскад/CRF); silver-val ~0.85–1.0 — завышен (тот же teacher). GLiNER на gold даёт небольшой плюс, на битых запросах поверх CRF иногда **портит** precision — как хвост нужен аккуратный гейт, не «всегда после CRF».

### Killer feature №2: контекстная достройка после цифры (мультикласс)

Задача: после токена-числа (`16`, `256`, `55`) модель выдаёт **распределение вероятностей**, что идёт дальше (или что дальше **ничего** не нужно):

| Класс | Пример | Смысл |
|---|---|---|
| `memory_gb` | samsung galaxy **16** → гб | память |
| `storage_tb` | ssd **1** → тб | накопитель |
| `size_inch` | телевизор **55** → дюйм | диагональ |
| `power_w` | чайник **2000** → вт | мощность |
| `model_code` | matepad **11** / айфон **16** | это часть модели, единицу **не** клеим |
| `NONE` | просто число / мусор | ничего не достраивать |

Это **мультиклассификация по контексту** (окно слева/справа от цифры), не обычный regex.

**Как отличить `16 айфон` от `16 гб`:**
- смотрим соседей: после/до числа стоит линейка/бренд (`iphone`, `galaxy`, `matepad`) → класс `model_code` / `NONE`;
- рядом категория «телефон/ноутбук» + типичные объёмы `{8,16,32,64,128,256,512}` и единицы нет → `memory_gb` с порогом;
- если единица уже написана (`16гб`) — класс не гадаем, забираем правило;
- всегда есть класс **NONE** и порог уверенности: лучше не достроить, чем приклеить ложные «гб» к модели iPhone 16.

**Модели (от простого к сильному):**
1. **Марков / n-граммы частот** — считаем по логам: `P(единица | бренд, категория, число)`. Быстро, интерпретируемо, отличный MVP.
2. **Логрег / бустинг на признаках** — токены ±2 вокруг числа, «типичный объём?», «есть бренд-линейка?».
3. **Лёгкая RNN/BiLSTM** (опционально) — если марков упирается в хвост; в online — только если влезаем в latency.

Цикл обучения тот же human-in-the-loop: обучили → прогнали 500–1000 → разобрали абсурд (`16 айфон`→гб) → доразметили → снова.

### Этап A — фикс ошибок ввода (до любых моделей)

Слой нормализации **перед** CRF/правилами:

| Тип ошибки | Пример | Как чиним |
|---|---|---|
| Опечатка единицы | `16гь`, `16гю` | словарь единиц + похожесть на клавиатуре (йцукен) + RapidFuzz → `гб` |
| Склейка | `16гб` | уже есть: отклеить `16 гб` |
| Усечённая единица | `16 г` | уже частично есть ≈ память |
| Раскладка / ё | `ё→е`, лат/кир `c/с` | таблица замен |
| Синонимы канона | `айфон↔iphone`, `гб↔gb` | `artifacts/dicts/synonyms.txt` |

Опечатки **не** отдаём на откуп CRF/GLiNER — сначала чиним строку.

### Этап B — модель «что после цифры» (киллер)

1. Собрать silver из запросов: все спаны `число (+ опционально единица)`.
2. Разметить / слабо разметить класс (`memory_gb` / `model_code` / `NONE` / …).
3. Обучить марковскую/частотную мультиклассовую модель + пороги.
4. Error analysis на кейсах вроде `айфон 16`, `matepad 11`, `galaxy 16`.
5. Встроить в каскад: правила → нормализация → **unit-completer** → CRF добивает остальное.

На выходе в JSON не «магия», а честно:
`attributes.memory = 16гб` **или** `entities.MODEL = iphone 16`, плюс `confidence`.

### Этап C — 0/1 RecSys (killer feature выдачи)

Когда факты стали чище (опечатки + достройка единиц):
- ручные пары запрос↔карточка (1 = всё из запроса есть в карточке, 0 = пусто/не то);
- обучение → прогон → правка абсурда → снова;
- выдача товаров опирается на покрытие фактов, а не только на кликовый шум.

### Этап D — GLiNER (потом)

Гибкий NER на хвосте/новых типах. **После** нормализации, unit-completer и 0/1.  
Не вместо CRF и не вместо фикса `гь`.

### Сводная таблица приоритетов

| # | Что | Статус | Зачем |
|---|---|---|---|
| **1** | Фикс опечаток + синонимы (`гь→гб`, каноны, гомоглифы, алиасы) | ✅ почти | Грязный ввод перестаёт ломать правила/CRF |
| **2** | Мультикласс «что после цифры» (марков → опц. RNN) | ⬜ следующий киллер | Достроить `16→гб` **или** понять, что это модель (`айфон 16`) |
| **3** | 0/1 RecSys на парах запрос↔карточка | ⬜ после B | Правильная **выдача** товаров |
| **4** | GLiNER точечно на хвост | ✅ обучен / ⬜ в прод | Гибкий NER; вшивать с гейтом после A+B |
| — | Золотая BIO → CRF v2 | ⬜ | Переобучение на чистой разметке команды |
| — | Починить `category_clf` (PlayStation→фен и т.п.) | ⬜ | Не ломать выдачу после NER |
| — | macOS / подпись | ⬜ | Сборка есть; Developer ID по возможности |

**Итог одной фразой:** опечатки и GLiNER-эксперименты закрыты; дальше — unit-completer после цифр, чистый gold→CRF v2, аккуратно вшить GLiNER в каскад, потом 0/1 для выдачи.
