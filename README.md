...
![Версия](https://img.shields.io/badge/версия-0.4.0-f20601?style=for-the-badge)
![Платформа](https://img.shields.io/badge/Windows-10%2F11-0078d4?style=for-the-badge&logo=windows)
![macOS](https://img.shields.io/badge/macOS-12%2B-000000?style=for-the-badge&logo=apple)
...
![FastAPI](https://img.shields.io/badge/FastAPI-сервис-009688?style=for-the-badge&logo=fastapi)

![NER](https://img.shields.io/badge/NLP-SpellFix_%2B_CRF_%2B_Markov-1c1c1e)
...
```
запрос ❯ SpellFix ❯ ПРАВИЛА (словари · регулярки · модели) ❯ CRF ❯ КЛАССИФИКАТОР БРЕНДА ❯ МАРКОВСКИЙ ТИПИЗАТОР
                                                                                                    │
                                                                  merge + нормализация → факты JSON, < 100 мс
```

| Слой | Что закрывает | Где |
|---|---|---|
| **SpellFix** | опечатки, гомоглифы (`c/с`), алиасы (`сони→sony`), `16гь→16 гб` | Qt .exe + Python extractor |
| **Правила и словари** | явные бренды, единицы, линейки моделей | 7 словарей + майнинг |
| **CRF** | морфология, порядок слов, границы спанов | silver BIO → `ner_crf.pkl` |
| **Классификатор бренда** | бренд не написан в тексте (73% кликов) | 67 классов, включая NO_BRAND / UNKNOWN |
| **Типизатор атрибутов** | тип ATTR: «16 гб» → память | марковская цепь (C++) / TF-IDF clf (Python) |

Тот же каскад доступен тремя способами: **нативное Qt-приложение**, **FastAPI** (`POST /extract`) и **CLI** (`python -m src.cli.cli`).
...
├── gliner/               # GLiNER zero-shot + fine-tune
├── general_study/        # совместный каскад rules→CRF→GLiNER
└── _legacy/              # архив ранних ноутбуков (не поддерживаются)
installer/                # Inno Setup: два установщика
docs/                     # презентации (XeLaTeX, фирменный стиль)
...
- **CMake + MSVC 2022** — сборка Windows; **CMake + Ninja + macdeployqt** — macOS
- **Inno Setup 6** — установщики Windows (~17 МБ)
- **Python 3.10+** — NER-пайплайн: pandas, scikit-learn, sklearn-crfsuite, pymorphy3, FastAPI
- **XeLaTeX** — презентации в фирменном стиле

## 🌐 FastAPI и CLI

```bash
# HTTP-сервис (тот же QueryEntityExtractor)
uvicorn src.service.app:app --host 0.0.0.0 --port 8000
# POST /extract  {"query":"телфон 16 гь"}
# GET  /health

# CLI smoke-тест
python -m src.cli.cli "ноутбук asus 16гь"
