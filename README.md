...
| # | Что | Зачем |
|---|---|---|
| **1** | Фикс опечаток + синонимы (`гь→гб`, каноны) | Грязный ввод перестаёт ломать правила/CRF |
| **2** | Мультикласс «что после цифры» (марков → опц. RNN) | Killer: достроить `16→гб` **или** понять, что это модель (`айфон 16`) |
| **3** | 0/1 RecSys на парах запрос↔карточка | Killer: правильная **выдача** товаров |
| **4** | GLiNER точечно на хвост | Гибкий NER, когда база уже стоит |
| — | Золотая BIO → CRF v2 | Переобучение на разметке команды |
| — | macOS / подпись | Сборка есть; Developer ID по возможности |

**Итог одной фразой:** сначала чиним строку и учим модель *достраивать или не достраивать* единицу после числа; потом 0/1 для выдачи; GLiNER — в конце как усиление NER, а не как замена всего пайплайна.
