...
Порядок дальше (осознанно, не всё сразу):

| Приоритет | Этап | Что делаем |
|---|---|---|
| **1** | Killer feature 0/1 (RecSys) | Ручные пары запрос↔карточка: **1** если всё из запроса есть в карточке, **0** если карточка пустая / не покрывает. Обучаем → прогон на 500–1000 → error analysis (правим абсурд) → дообучение → снова. Пока качество не устроит, потом разметка на большой выборке |
| **2** | Синонимы / нормализация | Словари канонов: `айфон↔iphone↔Apple`, `гб↔gb`, морфология цвета. Помогает и NER (единый вид фактов), и 0/1 (запрос и карточка начинают «видеть» друг друга). Не отдельная тяжёлая разметка alignment на каждое слово |
| **3** | GLiNER (потом) | Гибкий NER на хвосте / новых типах сущностей. Не вместо CRF сейчас: сначала 0/1 и синонимы, потом GLiNER точечно |
| — | Золотая BIO | 3×1500 запросов; срез Лизы уже есть — переобучение CRF v2 на gold |
| — | macOS / подпись | Сборка на Маке есть; ad-hoc codesign; Apple Developer ID — по возможности |

**Зачем так:** 0/1 сразу бьёт в выдачу товаров (killer feature). Синонимы дёшево усиливают и факты, и матч. GLiNER — мощный, но тяжёлый слой: его логично брать после того, как RecSys и нормализация уже стоят.
