...
% ———- жирный шеврон, как в шаблоне М.Тех (стрелка-галка, компактный) ———-
\newcommand{\chevron}{%
  \raisebox{-0.35ex}{\begin{tikzpicture}
    \draw[mvred, line width=2pt, line cap=round, line join=round]
      (0,0.22) -- (0.11,0.11) -- (0,0);
  \end{tikzpicture}}}
% шаг пайплайна: #1 fill, #2 text color, #3 текст
\newcommand{\pipestep}[3]{%
  \begin{tikzpicture}[baseline=(b.center)]
    \node[fill=#1, rounded corners=3pt, minimum height=1.0cm, text width=1.85cm,
          align=center, inner sep=3pt, font=\InterSemi\fontsize{7.5}{9}\selectfont, text=#2] (b) {#3};
  \end{tikzpicture}}
\newcommand{\pipestepc}[3]{%
  \begin{tikzpicture}[baseline=(b.center)]
    \node[fill=#1, rounded corners=3pt, minimum height=0.95cm, text width=1.62cm,
...
\newcommand{\pipestepq}[3]{%
  \begin{tikzpicture}[baseline=(b.center)]
    \node[fill=#1, rounded corners=3pt, minimum height=0.82cm, text width=1.42cm,
...
\begin{columns}[T]
\column{0.5\textwidth}
\vspace{2pt}\centering\includegraphics[width=0.98\textwidth,height=4.5cm,keepaspectratio]{01_query_length.png}
...
\begin{columns}[T]
\column{0.5\textwidth}
\vspace{2pt}\centering\includegraphics[width=0.98\textwidth,height=4.5cm,keepaspectratio]{04_brand_in_query_vs_click.png}
\column{0.47\textwidth}
...
\begin{columns}[T]
\column{0.5\textwidth}
\vspace{2pt}\centering\includegraphics[width=0.98\textwidth,height=4.5cm,keepaspectratio]{03_bio_class_distribution.png}
\column{0.47\textwidth}
...
\begin{columns}[T]
\column{0.5\textwidth}
\vspace{2pt}\centering\includegraphics[width=0.98\textwidth,height=4.4cm,keepaspectratio]{02_preprocess_effect.png}
\column{0.47\textwidth}
...
\vspace{2pt}\centering\includegraphics[width=0.98\textwidth,height=4.3cm,keepaspectratio]{model_tag/01_missing_model_problem.png}
...
\begin{tikzpicture}[node distance=0pt]
  ...
  \node[right=2pt of a] (c1) {\chevron};
  \node[right=2pt of c1] (b) {\pipestep{cardbg}{mvdark}{ПРАВИЛА\\\fontsize{6}{7}\selectfont словари · регулярки · модели}};
  \node[right=2pt of b] (c2) {\chevron};
  \node[right=2pt of c2] (d) {\pipestep{cardbg}{mvdark}{CRF\\\fontsize{6}{7}\selectfont разметка последовательности}};
  \node[right=2pt of d] (c3) {\chevron};
  \node[right=2pt of c3] (e) {\pipestep{cardbg}{mvdark}{КЛАССИФИКАТОР\\\fontsize{6}{7}\selectfont бренд, если не написан}};
  \node[right=2pt of e] (c4) {\chevron};
  \node[right=2pt of c4] (f) {\pipestep{mvdark}{white}{ТИПИЗАТОР\\\fontsize{6}{7}\selectfont марковская цепь}};
\end{tikzpicture}
\end{center}
\vspace{7pt}\par
\begin{center}
...
\begin{tikzpicture}[node distance=0pt]
  ...
  \node[right=2pt of p1] (a1) {\chevron};
  \node[right=2pt of a1] (p2) {\pipestepq{cardbg}{mvdark}{Пред-\\обработка}};
  \node[right=2pt of p2] (a2) {\chevron};
  \node[right=2pt of a2] (p3) {\pipestepq{mvred}{white}{Извлечение\\фактов}};
  \node[right=2pt of p3] (a3) {\chevron};
  \node[right=2pt of a3] (p4) {\pipestepq{cardbg}{mvdark}{Нормали-\\зация}};
  \node[right=2pt of p4] (a4) {\chevron};
  \node[right=2pt of a4] (p5) {\pipestepq{cardbg}{mvdark}{Каталог}};
  \node[right=2pt of p5] (a5) {\chevron};
  \node[right=2pt of a5] (p6) {\pipestepq{cardbg}{mvdark}{Ранжиро-\\вание}};
\end{tikzpicture}
\end{center}
\vspace{5pt}\par
...
\begin{columns}[T]
\column{0.5\textwidth}
\vspace{2pt}\centering\includegraphics[width=0.98\textwidth,height=4.4cm,keepaspectratio]{markov/03_markov_transitions.png}
\column{0.47\textwidth}
...
\begin{columns}[T]
\column{0.5\textwidth}
\vspace{2pt}\centering\includegraphics[width=0.98\textwidth,height=4.4cm,keepaspectratio]{markov/05_number_ambiguity.png}
\column{0.47\textwidth}
...
\begin{tikzpicture}[node distance=0pt]
  ...
  \node[right=2pt of m1] (mc1) {\chevron};
  \node[right=2pt of mc1] (m2) {\pipestep{mvred}{white}{Обучаем модель\\\fontsize{6}{7}\selectfont бустинг}};
  \node[right=2pt of m2] (mc2) {\chevron};
  \node[right=2pt of mc2] (m3) {\pipestep{cardbg}{mvdark}{Модель метит\\\fontsize{6}{7}\selectfont миллионы пар: 1 / 0}};
  \node[right=2pt of m3] (mc3) {\chevron};
  \node[right=2pt of mc3] (m4) {\pipestep{cardbg}{mvdark}{Где 1 — берём\\\fontsize{6}{7}\selectfont карточку в выдачу}};
\end{tikzpicture}
\end{center}
\vspace{8pt}\par
\begin{columns}[T]
...
% ============================================================
\begin{frame}[t]
\slidehead{приложение · разметка}{Приложение разметки: мастер из трёх этапов}
\graynote{КАК УСТРОЕНО}{Запрос разбит на блоки по центру, фон заблюрен. Этап 1: клавиши B/I/O, метка появляется над блоком, Backspace снимает и шагает назад. Enter — подтверждение этапа.}
\vspace{6pt}\par
\begin{columns}[T]
\column{0.49\textwidth}
\centering
\includegraphics[width=\textwidth]{apps/labeling_stage1_bio.png}\par\vspace{2pt}
{\smalltext\color{mvgray} Этап 1 — разметка B/I/O: активный блок в красной рамке, метки над блоками}
\column{0.49\textwidth}
\centering
\includegraphics[width=\textwidth]{apps/labeling_stage2_types.png}\par\vspace{2pt}
...
\graynote{ТРИ СБОРКИ}{Для Никиты, Некита и Лизы — по 1500 непересекающихся запросов. Если у атрибута выбрано «другое» — подтип вводится вручную. Пустая карточка в режиме 1/0 получает 0 автоматически.}
\vspace{6pt}\par
\begin{columns}[T]
\column{0.49\textwidth}
\centering
\includegraphics[width=\textwidth]{apps/labeling_stage3_subtypes.png}\par\vspace{2pt}
{\smalltext\color{mvgray} Этап 3 — подтип атрибута: у каждого описание и перевод; «другое» — ручной ввод}
\column{0.49\textwidth}
\centering
\includegraphics[width=\textwidth]{apps/labeling_match.png}\par\vspace{2pt}
