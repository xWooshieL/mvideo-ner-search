...
\vspace{5pt}\par
{\InterSemi\fontsize{8.5}{10.5}\selectfont Шаг 1 · расклейка и сепараторы}\par\vspace{3pt}
...
\par\vspace{7pt}
{\InterSemi\fontsize{8.5}{10.5}\selectfont Шаг 2 · разметка со словарями и подсказками}\par\vspace{3pt}
{\fontsize{9}{13}\selectfont
...
\par\vspace{7pt}
{\InterSemi\fontsize{8.5}{10.5}\selectfont Шаг 3 · готовые факты}\par\vspace{3pt}
...
\begin{columns}[T]
\column{0.52\textwidth}
{\InterSemi\fontsize{8}{10}\selectfont Название карточки $\to$ кандидаты}\par\vspace{4pt}
{\fontsize{7.6}{11}\selectfont
\wplain{Наушники} \ \wtag{mvdark}{Logitech} \ \wtag{okgreen}{G} \ \wtag{okgreen}{PRO} \ \wtag{okgreen}{X} \ \wtag{okgreen}{SE} \ \wplain{Black}}\par\vspace{4pt}
{\smalltext После бренда снимаем 1–4 коротких токена и получаем цепочку кандидатов:}\par\vspace{3pt}
{\renewcommand{\arraystretch}{1.1}\setlength{\tabcolsep}{5pt}
\begin{tabular}{|L{2.3cm}|C{2.3cm}|}
\hline
\theadl{Кандидат} & \thead{Встретился раз}\\
\hline
\tcelll{g} & \tcellc{213}\\
\hline
...
{\InterSemi\fontsize{8}{10}\selectfont Почему порог — 6 раз}\par\vspace{4pt}
{\smalltext
...
\vspace{5pt}\par
{\smalltext
\numdot{1}\ {\InterSemi Откуда метки, если разметки нет?} От регулярок-учителей. Регулярка «число + гб» сама знает, что это память, — прогоняем её по 60 тысячам запросов и каждой паре токенов записываем тип, который назначила регулярка.\par\vspace{3pt}
\numdot{2}\ {\InterSemi Что запоминает цепь.} Таблицу частот: пара «<число>|гб» встретилась 154 раза, и все 154 раза регулярка сказала «память». На инференсе цепь просто смотрит пару соседних токенов в спане.\par\vspace{3pt}
\numdot{3}\ {\InterSemi Зачем, если есть регулярка?} Цепь обобщает шире: срабатывает и на паре, которую регулярка целиком не поймала (опечатки, перестановки), и отвечает за доли миллисекунды.\par\vspace{3pt}
\numdot{4}\ Точность против правил-учителя — 0{,}62; в 36\% случаев цепь честно говорит «не знаю», но типы между собой почти не путает.}
\end{columns}
\slidefoot
\end{frame}
...
\begin{center}
\begin{tikzpicture}[
  bx/.style={rounded corners=3pt, minimum height=0.8cm, minimum width=1.9cm, align=center, font=\InterSemi\fontsize{7.5}{9}\selectfont},
  ar/.style={-{Stealth[length=2mm]}, line width=1.1pt, mvred}
]
  \def\R{1.55}
  \node[bx, fill=mvdark, text=white] (model) at (90:\R) {Модель};
  \node[bx, fill=mvred, text=white] (unc) at (0:\R+0.55) {Сомнения};
  \node[bx, fill=mvdark, text=white] (lab) at (270:\R) {Разметка};
  \node[bx, fill=mvred, text=white] (ret) at (180:\R+0.55) {Дообучение};
  \draw[ar] (55:\R+0.35) arc (55:35:\R+0.35);
  \draw[ar] (325:\R+0.35) arc (325:305:\R+0.35);
  \draw[ar] (235:\R+0.35) arc (235:215:\R+0.35);
  \draw[ar] (145:\R+0.35) arc (145:125:\R+0.35);
\end{tikzpicture}
\end{center}
...
\column{0.49\textwidth}
\centering
\includegraphics[width=\textwidth,height=5.2cm,keepaspectratio]{apps/cpp_label_stage1.png}\par\vspace{2pt}
{\smalltext\color{mvgray} Этап 1 — разметка B/I/O: активный блок в красной рамке, метки над блоками}
\column{0.49\textwidth}
\centering
\includegraphics[width=\textwidth,height=5.2cm,keepaspectratio]{apps/cpp_label_stage2.png}\par\vspace{2pt}
...
\column{0.49\textwidth}
\centering
\includegraphics[width=\textwidth,height=5.2cm,keepaspectratio]{apps/cpp_label_stage3.png}\par\vspace{2pt}
{\smalltext\color{mvgray} Этап 3 — подтип атрибута: у каждого описание и перевод; «другое» — ручной ввод}
\column{0.49\textwidth}
\centering
\includegraphics[width=\textwidth,height=5.2cm,keepaspectratio]{apps/cpp_label_match.png}\par\vspace{2pt}
...
\column{0.49\textwidth}
\centering
\includegraphics[width=\textwidth,height=5.2cm,keepaspectratio]{apps/cpp_mvp_search.png}\par\vspace{2pt}
{\smalltext\color{mvgray} Факты чипами + товары, отранжированные по фактам, с полосой релевантности}
\column{0.49\textwidth}
\centering
\includegraphics[width=\textwidth,height=5.2cm,keepaspectratio]{apps/cpp_mvp_json.png}\par\vspace{2pt}
...
\column{0.49\textwidth}
\centering
\includegraphics[width=\textwidth,height=5.2cm,keepaspectratio]{apps/cpp_mvp_stats.png}\par\vspace{2pt}
