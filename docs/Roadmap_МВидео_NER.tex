...
\begin{enumerate}
  ...
  \item Зафиксировать JSON: brand, category, attributes (значение + уверенность), entities, latency\_ms.
  ...
  \item Не верить, что <<данные пришли как надо>>: распаковать skus.pkl и проверить структуру, кодировку, обязательные поля.
\end{enumerate}
}{%
\begin{enumerate}
...
\textbf{skus.pkl живой:} 1.5\,ГБ, распаковался, внутри \textbf{546\,069 офферов}, кириллица читается без кракозябр, поля \emph{name / vendor / model / param} заполнены на 100\%. То есть по тексту данные пришли нормально, паниковать не о чем.\\[4pt]
\textbf{Но есть подвох:} картинок в пикле \textbf{нет вообще} --- поле \emph{picture} пустое у всех офферов. Так что идея <<брать цвет с фото>> --- это отдельный поход за картинками по id/url М.Видео, а не поле из коробки. Цвет как текстовый атрибут (\emph{Цвет\_search}) есть примерно у 2/3, у хвоста --- пусто, вот там фото и пригодится.
\end{tcolorbox}
\end{center}
...
\begin{enumerate}
  ...
  \item Прогнать CRF по gold: P/R/F1 \emph{по каждому атрибуту} + coverage, а не одну среднюю accuracy (она прячет провал на хвосте).
...
\begin{enumerate}
  ...
  \item Нормализация и снятие неоднозначности: айфон / iphone / Apple $\to$ один канон; <<красный>> --- это цвет, а <<Красный Октябрь>> --- уже бренд, и что именно имелось в виду, решает контекст, а не словарь в лоб.
...
На защите первым делом спросят: <<почему так, а не нейросеткой всё подряд / не LLM на всё>>. Ответ --- вот эта табличка. Смысл простой: ни один подход не закрывает все колонки сразу, поэтому в проде они уживаются вместе, а не воюют.
...
\toprule
\textbf{Подход} & \textbf{Хвост} & \textbf{Цена} & \textbf{Данные} & \textbf{Поддержка} & \textbf{Cold-start} \\
\midrule
Правила (regex+словари) & низко & очень низко & не нужны & средне & высоко \\
NER / CRF & средне & низко & средне & средне & низко \\
Трансформер (BERT) & высоко & средне & много & средне & низко \\
Мультимодал (текст+фото) & высоко & высоко & много & низко & низко \\
LLM few-shot & высоко & высоко & мало & высоко & очень высоко \\
\bottomrule
\end{tabular}\\[3pt]
{\tiny\color{inksoft}<<Цена>> --- стоимость инференса; <<Хвост>> --- качество на редких значениях; <<Данные>> --- сколько разметки нужно.}
\end{center}

\vspace{0.1cm}
{\color{mvred}\bfseries Вывод:} правильного <<одного>> подхода нет. Гибрид: правила и словари держат ядро и дают дешёвый бейзлайн, обученная модель (CRF, дальше --- BERT) добивает свободный текст и хвост, LLM затыкает cold-start и совсем редкие категории. Мы честно показываем этот компромисс <<покрытие $\leftrightarrow$ стоимость $\leftrightarrow$ поддерживаемость>>, а не делаем вид, что <<взяли самое модное и всё заработало>>.

\daysection{Растяжки --- если останется время (и на \emph{after})}
...
\begin{enumerate}
  \item По id/url оффера подтянуть фото с М.Видео (в pickle картинок нет --- надо ходить наружу).
  \item Vision-энкодер (SigLIP / CLIP) $\to$ достаём цвет, дальше --- принт и форму.
...
\begin{enumerate}
  ...
  \item Понимание, где мультимодалка реально нужна, а где это оверкилл ради оверкилла.
\end{enumerate}
}

\dayblock{+}{LLM на cold-start и длинный хвост}{%
\begin{enumerate}
  \item Новая категория без разметки $\to$ few-shot промпт со \textbf{structured output} (JSON строго по схеме).
  \item Загон в рамки: значения из справочника, порог уверенности, проверка <<откуда взяли значение>> --- иначе галлюцинации ломают фильтры.
  \item LLM-assisted разметка: модель пред-размечает, человек только верифицирует --- дешёвые лейблы, которыми потом кормим свою модель.
\end{enumerate}
}{%
\begin{enumerate}
  \item Ответ на <<а как вы заводите новую категорию за день, а не за месяц>>.
  \item Страховка от галлюцинаций описана заранее, а не постфактум на вопрос из зала.
\end{enumerate}
}

\section{Риски --- называем сами, раньше, чем спросят}
\secrule
На уровне <<взрослого>> ревью первым делом спрашивают про риски. Прятать их --- худшая стратегия; показываем сразу с мерой снижения.
\begin{enumerate}
  \item \textbf{Галлюцинации LLM} $\to$ ложные атрибуты ломают фильтры. Лечим structured output + порогом уверенности.
  \item \textbf{Цена на масштабе} $\to$ LLM на каждый из 546k офферов --- дорого. Лечим гибридом: LLM только на хвост и cold-start.
  \item \textbf{Eval врёт} $\to$ красивая средняя метрика прячет провал на редких категориях. Лечим gold по хвосту и метриками по атрибутам.
  \item \textbf{Дрейф} $\to$ новые SKU и категории каждый день. Нужны мониторинг и петля дообучения.
