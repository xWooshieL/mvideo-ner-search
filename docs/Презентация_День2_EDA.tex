...
\conclusion{Нашли \textbf{80+} запросов вида \texttt{dfhjxyfz gfytkm} — это «варочная панель», набранное в латинской раскладке. Поиск обязан их авто-исправлять.}
\vspace{5pt}\par
{\fontsize{5.7}{7.4}\selectfont\setlength{\tabcolsep}{3pt}\renewcommand{\arraystretch}{1.15}
\begin{tabular}{@{}p{0.315\textwidth}p{0.315\textwidth}p{0.315\textwidth}@{}}
\texttt{lkz djkjc aty gkjqrf gkjqrf}\,{\color{mvred}$\to$}\,\textbf{для волос фен плойка плойка} & \texttt{dcnhfbbdftvsq [jkjlbkmybr}\,{\color{mvred}$\to$}\,\textbf{встраииваемый холодильник} & \texttt{rkfdbfnehf buhjfdz}\,{\color{mvred}$\to$}\,\textbf{клавиатура игроавя} \\[1.3pt]
\texttt{pfhzlyst ecnhjqcndf lkz frrevekjnjhjd fdnj}\,{\color{mvred}$\to$}\,\textbf{зарядные устройства для аккумулоторов авто} & \texttt{dcnhjtyyfz gjceljvjqrf}\,{\color{mvred}$\to$}\,\textbf{встроенная посудомойка} & \texttt{rjkjyrb rjvgm.nthyst}\,{\color{mvred}$\to$}\,\textbf{колонки компьютерные} \\[1.3pt]
\texttt{dfhjxyfz gfytkm ,tkjt cntrkh}\,{\color{mvred}$\to$}\,\textbf{варочная панель белое стеклр} & \texttt{dcnhjtyysq [jkjlbkmybr}\,{\color{mvred}$\to$}\,\textbf{встроенный холодильник} & \texttt{rjv,bybhjdfyyfz gkbnf}\,{\color{mvred}$\to$}\,\textbf{комбинированная плита} \\[1.3pt]
\texttt{dbltjrfvthf lkz gjvtotybz}\,{\color{mvred}$\to$}\,\textbf{видеокамера для помещения} & \texttt{ufpjdfz gkbnf}\,{\color{mvred}$\to$}\,\textbf{газовая плита} & \texttt{rjvgm.nthyst rjkjyrb}\,{\color{mvred}$\to$}\,\textbf{компьютерные колонки} \\[1.3pt]
\texttt{dcnhfbdftvfz cnbhfkmyfz vfibyf}\,{\color{mvred}$\to$}\,\textbf{встраиваемая стиральная машина} & \texttt{ufqrjdthn htldthu}\,{\color{mvred}$\to$}\,\textbf{гайковерт редверг} & \texttt{rjat pthyjdjq}\,{\color{mvred}$\to$}\,\textbf{кофе зерновой} \\[1.3pt]
\texttt{le[jdjq irfa 'ktrnhbxtcrbq}\,{\color{mvred}$\to$}\,\textbf{духовой шкаф электрический} & \texttt{ukflbkmyfz ljcrf}\,{\color{mvred}$\to$}\,\textbf{гладильная доска} & \texttt{rhtckj rjvgm.nthyjt}\,{\color{mvred}$\to$}\,\textbf{кресло компьютерное} \\[1.3pt]
\texttt{pfhzlyjt ecnhjqcndj czjvb}\,{\color{mvred}$\to$}\,\textbf{зарядное устройство сяоми} & \texttt{lhtkm ,eh}\,{\color{mvred}$\to$}\,\textbf{дрель бур} & \texttt{vbrhjdjkyjdrf rfhhthf}\,{\color{mvred}$\to$}\,\textbf{микроволновка каррера} \\[1.3pt]
\texttt{rf,tkm lkz rkfdbfnehs}\,{\color{mvred}$\to$}\,\textbf{кабель для клавиатуры} & \texttt{pfdfhjxysq xfqybr}\,{\color{mvred}$\to$}\,\textbf{заварочный чайник} & \texttt{vbrhjdjkyjdrf njib,f}\,{\color{mvred}$\to$}\,\textbf{микроволновка тошиба} \\[1.3pt]
\texttt{gsktcjc dthnbrfkmysq ghjdjlyjq}\,{\color{mvred}$\to$}\,\textbf{пылесос вертикальный проводной} & \texttt{pfhzlyjt ecnhjqcndj}\,{\color{mvred}$\to$}\,\textbf{зарядное устройство} & \texttt{vj,bklmsyq rjylbwbjyth}\,{\color{mvred}$\to$}\,\textbf{мобилдьынй кондиционер} \\[1.3pt]
\texttt{nhbvvth lkz yjcf}\,{\color{mvred}$\to$}\,\textbf{триммер для носа} & \texttt{buhjdjt rhtckj}\,{\color{mvred}$\to$}\,\textbf{игровое кресло} & \texttt{vjhjpbkmyfz rfvthf}\,{\color{mvred}$\to$}\,\textbf{морозильная камера} \\[1.3pt]
\texttt{'ktrnhbxtcrfz gkbnf akfvf}\,{\color{mvred}$\to$}\,\textbf{электрическая плита флама} & \texttt{BUHJDJQ HEKM}\,{\color{mvred}$\to$}\,\textbf{игровой руль} & \texttt{VJ.OBQ GSKTCJC}\,{\color{mvred}$\to$}\,\textbf{моющий пылесос} \\[1.3pt]
\texttt{,TUJDFZ LJHJ;RF}\,{\color{mvred}$\to$}\,\textbf{беговая дорожка} & \texttt{Bosch bfl}\,{\color{mvred}$\to$}\,\textbf{ищыср иад} & \texttt{vsim ,tcghjdjlyfz}\,{\color{mvred}$\to$}\,\textbf{мышь беспроводная} \\[1.3pt]
\texttt{,tcghjdjlyjq gsktcjc}\,{\color{mvred}$\to$}\,\textbf{беспроводной пылесос} & \texttt{bosch HBG}\,{\color{mvred}$\to$}\,\textbf{ищыср рип} & \texttt{yfgjkmyst xfcs}\,{\color{mvred}$\to$}\,\textbf{напольные часы} \\[1.3pt]
\texttt{dfhjxyfz gfytkm}\,{\color{mvred}$\to$}\,\textbf{варочная панель} & \texttt{Bosch WDS}\,{\color{mvred}$\to$}\,\textbf{ищыср цвы} & \texttt{yfcnjkmyfz gkbnf}\,{\color{mvred}$\to$}\,\textbf{настольная плита} \\[1.3pt]
\texttt{dfhjxyfz gjdth[yjcnm}\,{\color{mvred}$\to$}\,\textbf{варочная поверхность} & \texttt{Bosch wgb}\,{\color{mvred}$\to$}\,\textbf{ищыср цпи} & \texttt{jbl xtrem}\,{\color{mvred}$\to$}\,\textbf{оид чекуь} \\[1.3pt]
\texttt{dtynbkznjh yfcnjkmysq}\,{\color{mvred}$\to$}\,\textbf{вентилятор настольный} & \texttt{Bosch smv}\,{\color{mvred}$\to$}\,\textbf{ищыср ыьм} & \texttt{jngfhbdfntkm kitfort}\,{\color{mvred}$\to$}\,\textbf{отпариватель лшеащке} \\[1.3pt]
\texttt{dthnbrfkmysq gsktcjc}\,{\color{mvred}$\to$}\,\textbf{вертикальный пылесос} & \texttt{rkfdbfnehf ,tkfz}\,{\color{mvred}$\to$}\,\textbf{клавиатура белая} & \texttt{jngfhbdfntkm hexyjq}\,{\color{mvred}$\to$}\,\textbf{отпариватель ручной} \\[1.3pt]
\end{tabular}}
\vspace{3pt}\par
{\smalltext\color{mvgray} Детект дешёвый — одна таблица QWERTY$\leftrightarrow$ЙЦУКЕН. Ловится в «голове» каскада рядом с regex; без него такой запрос не находит вообще ничего.}
\slidefoot
\end{frame}
% ============================================================
...
\column{0.48\textwidth}
{\InterSemi\fontsize{8.5}{10.5}\selectfont Как используем скор}\par\vspace{7pt}
\begin{tikzpicture}[
  ...
  arr/.style={-{Stealth[length=2mm]}, line width=0.9pt, mvgray}
]
  \node[st] (s1) {NER достал факты $\to$ фильтр и буст в выдаче};
  \node[st, below=4mm of s1] (s2) {$P(\text{match}=1 \mid \text{query},\ \text{sku\_text})$ — реранк поверх};
  \node[st, below=4mm of s2] (s3) {модель: градиентный бустинг (CatBoost / XGBoost) на простых признаках};
  \draw[arr] (s1) -- (s2);
  \draw[arr] (s2) -- (s3);
\end{tikzpicture}
\end{columns}
