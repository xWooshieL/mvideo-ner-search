...
\begin{columns}[T]
\column{0.5\textwidth}
\vspace{2pt}\centering\includegraphics[width=0.96\textwidth]{02_top_queries.png}
...
\vspace{6pt}\par
{\smalltext
\numdot{1}\ Отсюда обязателен слой \textbf{brand-классификатора} по кликам: «айфон 15» $\to$ Apple, хотя слова «Apple» нет.\par\vspace{4pt}
\numdot{2}\ Клики — не мусор, а обучающий сигнал: majority-бренд запроса даёт метку без ручной разметки.\par\vspace{4pt}
\numdot{3}\ Ограничение: \textbf{17\%} запросов кликают $\ge$2 бренда — берём majority с весом по позиции.}
\end{columns}
\slidefoot
\end{frame}
...
\column{0.47\textwidth}
\conclusion{Токены p50 / p90 / p99 = \textbf{2 / 4 / 8}. Запрос почти всегда $\le$ 8 слов — CRF летает на CPU под SLA.}
\vspace{6pt}\par
{\smalltext
\numdot{1}\ Weak-разметка даёт $\ge$1 сущность у \textbf{$\sim$80\%} запросов: BRAND $\sim$46\%, CATEGORY $\sim$59\%, ATTR $\sim$7\%.\par\vspace{4pt}
\numdot{2}\ У \textbf{$\sim$27\%} запросов есть \texttt{I-*} — многословные сущности, нужны именно границы span (BIO).\par\vspace{4pt}
\numdot{3}\ Где weak нашёл BRAND, согласие с брендом клика \textbf{$\sim$83\%} — общий сигнал для multi-task.}
\end{columns}
\slidefoot
\end{frame}
...
\hline
\tcelll{\hnum{H6.} Бренд обычно написан в запросе} & \tcellc{Нет} & \tcellc{только 27\% — нужен classifier}\\
\hline
\end{tabular}}
\vspace{5pt}\par
...
\column{0.48\textwidth}
{\InterSemi\fontsize{8.5}{10.5}\selectfont Как используем скор}\par\vspace{5pt}
\begin{tikzpicture}[
  st/.style={rounded corners=2pt, fill=cardbg, inner sep=5.5pt, text width=\dimexpr\textwidth-55pt\relax, font=\fontsize{7.5}{9.5}\selectfont},
...
% ============================================================
\begin{frame}[t]
\slidehead{честные метрики · smoke-test}{CRF на weak BIO: где мы сейчас}
\conclusion{Metrics на silver-метках оптимистичны — это ориентир, а не финальная оценка. Нужен mini-gold 200--500 запросов.}
\vspace{8pt}\par
\begin{columns}[T]
\column{0.42\textwidth}
{\InterSemi\fontsize{8.5}{10.5}\selectfont Smoke-test на семпле}\par\vspace{5pt}
\begin{tikzpicture}[
...
\numdot{2}\ CATEGORY слабее (0.78) — формулировки разнообразнее, тут нужен либо трансформер, либо больше weak-данных.\par\vspace{4pt}
