...
\newcommand{\tcell}[1]{{\ttext\color{mvdark}#1}}
...
\begin{tabular}{|p{6.4cm}|p{2.0cm}|p{4.0cm}|}
\hline
\thead{Гипотеза} & \thead{Статус} & \thead{Комментарий}\\
\hline
\tcell{H1. Запросы — короткие категорийные интенты} & \tacc{Да} & \tcell{топы — чистые категории}\\
\hline
\tcell{H2. Клики концентрируются в топе выдачи} & \tacc{Да} & \tcell{медиана позиции = 4}\\
\hline
\tcell{H3. Бренд в запросе $\Rightarrow$ клик того же бренда} & \tcell{В основном} & \tcell{heatmap диагональна}\\
\hline
\tcell{H4. Цена сильно влияет на позицию клика} & \tcell{Нет} & \tcell{hexbin: связь слабая}\\
\hline
\tcell{H5. Атрибуты пишут единообразно} & \tcell{Нет} & \tcell{«16гб», «16 gb», «16 ГБ»}\\
\hline
\end{tabular}}
\vspace{5pt}\par
...
\hline
\thead{Риск} & \thead{Последствие} & \thead{Меры снижения}\\
\hline
\tcell{«Соответствие» размыто} & \tcell{«dyson» $\to$ пылесос Dyson: 1 или 0?} & \tcell{жёсткий гайд разметки до старта}\\
\hline
\tcell{100 примеров маловато} & \tcell{модель выучит шум выборки} & \tcell{300--700 меток в день + согласие}\\
\hline
\tcell{Самореференция} & \tcell{ошибки размножаются на своих метках} & \tcell{holdout gold вне авторазметки}\\
\hline
\tcell{Путаница с NER} & \tcell{это query$\leftrightarrow$SKU, не BIO на словах} & \tcell{отдельные пайплайны и метрики}\\
\hline
\end{tabular}}
\vspace{5pt}\par
